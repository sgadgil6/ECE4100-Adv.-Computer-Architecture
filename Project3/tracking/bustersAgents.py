...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        globalMaxPrb = float("-inf")
        globalMaxPos = None
        for i in range (0, len(livingGhostPositionDistributions)):
            currMaxPos = livingGhostPositionDistributions[i].argMax()
            currMaxPrb = livingGhostPositionDistributions[i][currMaxPos]
            if currMaxPrb > globalMaxPrb:
                globalMaxPrb = currMaxPrb
                globalMaxPos = currMaxPos

        oldMazeDist = self.distancer.getDistance(globalMaxPos, pacmanPosition)
        diff = float("-inf")
        finalAction = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            newMazeDist = self.distancer.getDistance(globalMaxPos, successorPosition)
            currDiff = oldMazeDist - newMazeDist
            if currDiff > diff:
                diff = currDiff
                finalAction = action
