    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                allPossible[newPos] = (prob * self.beliefs[p]) + allPossible[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        avg_Particles = self.numParticles / len(self.legalPositions)
        self.particle_list = []
        for i in self.legalPositions:
            for j in range(0, avg_Particles + 1):
                self.particle_list.append(i)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for i in self.legalPositions:
                weighted_list[i] = beliefs[i] * emissionModel[util.manhattanDistance(pacmanPosition, i)] + weighted_list[i]
            if weighted_list.totalCount() != 0:
                for i in range(0, self.numParticles):
                    resample = util.sample(weighted_list)
                    temp_particle_list.append(resample)
    ...
    def elapseTime(self, gameState):
        ...
        belief distribution
        """
        temp_particle_list = self.particle_list
        self.particle_list = []
        for i in temp_particle_list:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, i))
            self.particle_list.append(util.sample(newPosDist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while num_particles <= self.numParticles:
            for i in list(product(self.legalPositions, repeat = self.numGhosts)):
                if num_particles <= self.numParticles:
                    self.particle_list.append(i)
                    num_particles += 1
        ...
        for i in range(0, self.numParticles):
            ...
            for j in range(self.numGhosts):
                if noisyDistances[j] is None:
                    new_particle = self.getParticleWithGhostInJail(self.particle_list[i], j)
                else:
                    weight *= emissionModels[j][util.manhattanDistance(pacmanPosition, self.particle_list[i][j])]

            if len(new_particle) == 0:
                weighted_list[self.particle_list[i]] = weighted_list[self.particle_list[i]] + weight
            else:
                weighted_list[new_particle] = weighted_list[new_particle] + weight
